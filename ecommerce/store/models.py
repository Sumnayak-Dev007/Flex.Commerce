...
def get_profile_path(request,filename):
    original_filename=filename
    nowTime=datetime.datetime.now().strftime('%Y%m%d%H:%M:%S')
    filename = "%s%s" % (nowTime,original_filename)
...
class Product(models.Model):
    ...
    slug=models.CharField(max_length=150,null=False,blank=False)
    name=models.CharField(max_length=150,null=False,blank=False)
    product_image=models.ImageField(storage=MediaCloudinaryStorage(), upload_to=get_file_path,null=True,blank=True)
...
class ProfileUser(models.Model):
    user = models.OneToOneField(User,on_delete=models.CASCADE)
    created_at = models.DateTimeField(auto_now_add=True)
    image = models.ImageField(storage=MediaCloudinaryStorage(), default = "default.png", upload_to=get_profile_path,validators=[FileExtensionValidator(['png','jpg'])])
